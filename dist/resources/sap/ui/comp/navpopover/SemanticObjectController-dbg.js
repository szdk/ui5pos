...
sap.ui.define([
	...
	SemanticObjectController.getDistinctSemanticObjects = function() {
		...
		if (!SemanticObjectController.oPromise) {
			SemanticObjectController.oPromise = Factory.getService("CrossApplicationNavigation", true).then(function(oCrossAppNav) {
				if (!oCrossAppNav) {
				...
				return oCrossAppNav.getDistinctSemanticObjects().then(function(aSemanticObjects) {
	...
	SemanticObjectController.getNavigationTargetActions = function(oSemanticObjects) {
		...
		if (!SemanticObjectController.oPromiseLinks) {
			var oCrossAppNavPromise = Factory.getService("CrossApplicationNavigation", true);
			var oURLParsingPromise = Factory.getService("URLParsing", true);

			SemanticObjectController.oPromiseLinks = Promise.all([oCrossAppNavPromise, oURLParsingPromise]).then(function(aValues) {
				var oCrossAppNav = aValues[0];
				...
				return oCrossAppNav.getLinks(aParams).then(function(aLinksOfSemanticObjects) {
