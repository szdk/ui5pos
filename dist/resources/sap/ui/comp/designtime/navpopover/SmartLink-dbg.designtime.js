...
sap.ui.define([
	...
	var fnGetEnableAvailableActionsPersonalizationForNavigationPopoverHandler = function(oNavigationPopoverHandler) {
		var oModel = oNavigationPopoverHandler.getModel("$sapuicompNavigationPopoverHandler");
		return oNavigationPopoverHandler._initModel().then(function() {
			return oNavigationPopoverHandler._getEnableAvailableActionsPersonalization(oModel.getProperty("/availableActions"));
		});
	};

	var fnStart = function(oNavigationPopoverHandler) {
		fnGetEnableAvailableActionsPersonalizationForNavigationPopoverHandler(oNavigationPopoverHandler).then(function(bEnableAvailableActionsPersonalization) {
			if (!oNavigationPopoverHandler._oRTANavigationHandling.enabled) {
				oNavigationPopoverHandler.setEnableAvailableActionsPersonalization(false);
				oNavigationPopoverHandler._oRTANavigationHandling.initialValue = bEnableAvailableActionsPersonalization;
				oNavigationPopoverHandler._oRTANavigationHandling.enabled = true;
			}
		});
	};

	var fnStop = function(oNavigationPopoverHandler) {
		if (oNavigationPopoverHandler._oRTANavigationHandling.enabled) {
			oNavigationPopoverHandler.setEnableAvailableActionsPersonalization(oNavigationPopoverHandler._oRTANavigationHandling.initialValue);
			oNavigationPopoverHandler._oRTANavigationHandling.enabled = false;
		}
	};
					...
					getStableElements: function(oObjectIdentifier) {
						...
						oObjectIdentifier.fireEvent("ObjectIdentifier.designtime", {
							...
							registerNavigationPopoverHandler: function(oNavigationPopoverHandler_) {
								...
								fnStart(oNavigationPopoverHandler);
							}
						});
	...
	return {
		...
		tool: {
			start: function(oSmartLink) {
				fnStart(oSmartLink.getNavigationPopoverHandler());
			},
			stop: function(oSmartLink) {
				fnStop(oSmartLink.getNavigationPopoverHandler());
			}
		},
		annotations: {
			/**
