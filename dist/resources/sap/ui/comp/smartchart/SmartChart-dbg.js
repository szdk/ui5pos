...
sap.ui.define([
	...
	SmartChart.prototype._addDrillUpDownButtons = function() {
		...
		if (this.getShowDrillButtons()) {
			...
			this._oDrillDownButton = new OverflowToolbarButton(this.getId() + "-btnDrillDown", {
				...
				press: function(oEvent) {
					if (this._oDrilldownPopover && this._oDrilldownPopover.isOpen()) {
						this._oDrilldownPopover.close();
					} else {
	...
	SmartChart.prototype._drillDown = function(oEvent, oSource) {
		...
		var oPopover, aIgnoreDimensions, aDimensions, oDimension, oList, oSearchField, i, oViewField;

		if (this._oChart) {
			if ((!ResponsivePopover || !List || !SearchField || !StandardListItem) && !this._bDrillDownRequested) {
				ResponsivePopover = sap.ui.require("sap/m/ResponsivePopover");
				List = sap.ui.require("sap/m/List");
				SearchField = sap.ui.require("sap/m/SearchField");
				StandardListItem = sap.ui.require("sap/m/StandardListItem");
				if (!ResponsivePopover || !List || !SearchField || !StandardListItem) {
					sap.ui.require([
						"sap/m/ResponsivePopover", "sap/m/List", "sap/m/SearchField", "sap/m/StandardListItem"
			...
			if (!this._oDrilldownPopover) {
				var oItemTemplate = new StandardListItem({
					title: "{$smartChartDrilldown>text}"
				});

				oList = new List({
					mode: ListMode.SingleSelectMaster,
					items: {
						path: "$smartChartDrilldown>/items",
						template: oItemTemplate
					},
					selectionChange: function(oEvent) {
						if (oEvent && oEvent.mParameters && oEvent.mParameters.listItem) {

							var sLabel = oEvent.mParameters.listItem.getTitle();
							var aDimensions = this._getSortedDimensions();
							var aFoundDim = aDimensions.filter(function(oDimension) {
								return oDimension.getLabel() === sLabel;
							});
				...
				oPopover = new ResponsivePopover({
					id: this.getId() + "-btnChartDrilldownPopover",
					contentWidth: "25rem",
					contentHeight: "20rem",
				...
				// On a Phone the Dialog renderer is used and the searchField as subHeader will not work. In this case we add the search field into the content.
				...
				this._oDrilldownPopover = oPopover;
			} else {
				oPopover = this._oDrilldownPopover;
				oSearchField = oPopover.getSubHeader() || oPopover.getContent()[0];
			}
			...
			oPopover.setModel(new JSONModel(oData), "$smartChartDrilldown");

			oSearchField.setValue("");
			oSearchField.setVisible(oData.items.length >= 7);
	...
	function _DrillDownLoaded(fnResponsivePopover, fnList, fnSearchField, fnStandardListItem) {
		ResponsivePopover = fnResponsivePopover;
		List = fnList;
	...
	SmartChart.prototype._createChartTypeButton = function() {
		...
		var oChartTypeButton = new OverflowToolbarButton(this.getId() + "-btnChartType", {
			...
			press: function(oEvent) {
				if (this._oChartTypePopover && this._oChartTypePopover.isOpen()) {
					this._oChartTypePopover.close();
				} else 	{
	...
	SmartChart.prototype._displayChartTypes = function(oEvent, oSource) {
		...
		var oPopover, oList, oSearchField;

		if (this._oChart && (oEvent || oSource)) {

			if ((!ResponsivePopover || !List || !SearchField || !StandardListItem) && !this._bChartTypesRequested) {
				ResponsivePopover = sap.ui.require("sap/m/ResponsivePopover");
				List = sap.ui.require("sap/m/List");
				SearchField = sap.ui.require("sap/m/SearchField");
				StandardListItem = sap.ui.require("sap/m/StandardListItem");
			...
			if (!this._oChartTypePopover) {
				...
				oList = new List({
					...
					selectionChange: function(oEvent) {
						...
						this._rerenderControlInFullscreen();
						oPopover.close();
					}.bind(this)
				});
				...
				oPopover = new ResponsivePopover({
					id: this.getId() + "-btnChartTypePopover",
					contentWidth: "25rem",
					contentHeight: "20rem",
					placement: PlacementType.VerticalPreferredBottom
				});
				...
				this._oChartTypePopover = oPopover;
			} else {
				oPopover = this._oChartTypePopover;
				oSearchField = oPopover.getSubHeader() || oPopover.getContent()[0];
			}

			oPopover.setModel(this.getModel("$smartChartTypes"), "$smartChartTypes");

			// reset existing filter when reopen the popover
			...
			oSearchField.setVisible(oPopover.getModel("$smartChartTypes").oData.items.length >= 7);
	...
	function _ChartTypesLoaded(fnResponsivePopover, fnList, fnSearchField, fnStandardListItem) {
		ResponsivePopover = fnResponsivePopover;
		List = fnList;
		SearchField = fnSearchField;
		StandardListItem = fnStandardListItem;
	...
	SmartChart.prototype._getChartPersonalisationData = function() {
		...
		if (this._oCurrentVariant.filter) {
			this._oCurrentVariant.filter.filterItems.forEach(function(oModelItem) {
				...
				if (oChartField && oChartField.filterType && oChartField.filterType === "time") {
					...
					var bDateTimeOffset = oChartField && oChartField.filterType === "datetime";
